...
if __name__ == '__main__':
    ...
    audio_files = find_recursive(args.root_audio, ext='.mp4')
    for audio_path in audio_files:
        frame_path = audio_path.replace(args.root_audio, args.root_frame)
        frame_file_path, fileName = os.path.split(frame_path)
        frame_files = glob.glob(frame_file_path + '/*.jpg')
    ...
    #Pradyumna modified to generate a sample val set
    #n_train = int(len(infos) * 0.8)
    #random.shuffle(infos)
    #trainset = infos[0:n_train]
    
    valset = infos[0:len(infos)]
    for name, subset in zip(['val'], [valset]):
        filename = '{}.csv'.format(os.path.join(args.path_output, name))
        with open(filename, 'w') as f:
            for item in subset:
                f.write(item + '\n')
